...
const BUDGET_BUCKETS: { id: string; label: string; test: (n?: number | null) => boolean }[] = [
  { id: "all", label: "Όλα", test: () => true },
  { id: "u150", label: "≤ 150k", test: (n) => n != null && n <= 150_000 },
  { id: "150-700", label: "150k – 700k", test: (n) => n != null && n > 150_000 && n <= 700_000 },
  { id: "700-3000", label: "700k – 3M", test: (n) => n != null && n > 700_000 },
];
  ...
  const results = useMemo(() => {
    ...
    return data.tenders
      .filter((t) => {
        ...
        if (!b.test(t.amount)) return false;
        if (text && !`${t.title} ${t.org}`.toLowerCase().includes(text)) return false;
        return true;
      })
      .sort((a, b2) => (a.issueDate < b2.issueDate ? 1 : -1));
  }, [q, types, bucket]);
  ...
  return (
    <>
      <div className="mt-4 rounded-xl border border-orange-200 bg-orange-50/70 px-4 py-3 text-[13px] text-orange-900">
        ...
        όλη η Ελλάδα, τελευταίες ~35 μέρες. Η ακριβής καταληκτική προσφορών είναι μέσα στη διακήρυξη.
      </div>
      ...
      <div className="mt-5 space-y-3">
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          placeholder="Αναζήτηση σε αντικείμενο ή φορέα (π.χ. «Αγρίνιο», «οδικό», «ύδρευση»)…"
          className="w-full max-w-md rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm shadow-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-200"
        />
        <FilterRow label="Τύπος">
          <Toggle active={types.has("Έργα")} onClick={() => toggleType("Έργα")}>🏗 Έργα</Toggle>
          <Toggle active={types.has("Μελέτες")} onClick={() => toggleType("Μελέτες")}>📐 Μελέτες</Toggle>
        </FilterRow>
        <FilterRow label="Π/Υ">
          {BUDGET_BUCKETS.map((b) => (
            <Toggle key={b.id} active={bucket === b.id} onClick={() => setBucket(b.id)}>{b.label}</Toggle>
          ))}
        </FilterRow>
      </div>
